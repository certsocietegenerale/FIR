...
def migrate_artifacts(apps, schema_editor):
    ...
    for old in olds:
        artifact = Artifact.objects.using(db_alias).create(
            id=old.id, type=old.type, value=old.value
        )
...
def migrate_files(apps, schema_editor):
    ...
    for old in olds:
        file = File.objects.using(db_alias).create(
            id=old.id, description=old.description, date=old.date, file=old.file
        )
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(migrate_files, atomic=False),
    ]
