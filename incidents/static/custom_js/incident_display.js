...
function parseStringTemplate(str, obj) {
  ...
  let parameters = args.map((argument_with_params) => {
    ...
    if (obj[argument] === undefined) {
      ...
      // strigs: allow custom-made strings manipulation
      if (
...
async function loadDynamicCSS() {
  ...
  for (elem of response["results"]) {
    name = elem["name"].toLowerCase().replace(/[^a-z0-9]+/g, "");
    color = elem["color"].toLowerCase().replace(/[^a-z0-9#]+/g, "");
...
async function refresh_display(div) {
  const request = await fetch(`${div.dataset.url}`, {
    headers: { Accept: "application/json" },
  });
  ...
  for (let entry of response["results"]) {
    ...
    for (i in entry["concerned_business_lines"]) {
      entry["concerned_business_lines"][i] = entry["concerned_business_lines"][
        i
      ]
        .split(" > ")
        .pop();
  ...
  if (response["total_pages"] != 1 && pagination_template) {
