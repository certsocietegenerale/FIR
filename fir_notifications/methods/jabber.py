...
class XmppMethod(NotificationMethod):
    ...
    name = "xmpp"
    verbose_name = "XMPP"
    options = {"jid": forms.CharField(max_length=100, label=_("Jabber ID"))}
    ...
    def __init__(self):
        ...
        if self.client.connect(server=self.connection_tuple, use_srv=self.use_srv):
            self.client.auth(
                self.jid.getNode(), self.password, resource=self.jid.getResource()
            )
    ...
    def send(self, event, users, instance, paths):
        if not self._ensure_connection():
            logging.getLogger("FIR").error(
                f"Cannot contact the XMPP server: {self.server}"
            )
            return
        for user, templates in users.items():
            ...
            message = xmpp.protocol.Message(
                jid,
                body=params["short_description"].encode("utf-8"),
    ...
    def configured(self, user):
        return (
            super(XmppMethod, self).configured(user) and self._get_jid(user) is not None
        )
