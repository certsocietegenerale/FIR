...
def get_perm_id():
    ...
    perm_obj = Permission.objects.get(
        content_type__app_label="incidents", codename="view_incidents"
    )
...
def get_user_templates(event, business_lines):
    global_users = User.objects.filter(
        models.Q(groups__permissions=get_perm_id())
        | models.Q(user_permissions=get_perm_id())
    ...
    business_lines = {
        bl: bl.get_ancestors()
        for bl in BusinessLine.objects.filter(path__in=business_lines).order_by("depth")
    }
    depth = 1
    all_templates = {}
    while len(business_lines):
        for lower in list(business_lines.keys()):
            ...
            if len(templates):
                ...
                role_users = (
                    User.objects.filter(accesscontrolentry__business_line=current_bl)
                    .filter(accesscontrolentry__role__permissions=get_perm_id())
                    .distinct()
                )
...
def handle_notification(content_type, instance, business_lines, event):
    ...
    try:
        ...
        logging.getLogger("FIR").error("Unknown content type", exc_info=True)
        return
    try:
        instance = model.objects.get(id=instance)
    except model.DoesNotExist:
        logging.getLogger("FIR").error("Unknown instance", exc_info=True)
        return
