...
async function setIncidentCount(div, with_attribute_only = false) {
  const date_filter = getDateFilterArg(div);
  if (!date_filter) {
    return false;
  }
  const other_filters = getOtherFiltersArg(with_attribute_only);
  ...
  div.textContent = response["incidents"];
}
...
function refresh_stats() {
  ...
  if (incAttributesCount) {
    setIncidentCount(incAttributesCount, true);
  }
}
...
async function generate_variation_table(div) {
  ...
  newDescription.textContent = "Current month (last 30 days)";
  newDescription.classList.add("text-end");
  trNew.appendChild(newDescription);

  varDescription = document.createElement("th");
  varDescription.textContent =
    "Variation from the previous month (30-60 days ago)";
...
async function generate_multiple_donut_chart(div) {
  ...
  g.append("path")
    ...
    .attr("title", function (d) {
      return d.data.value; // Append a text element (tooltip) to display the value of each slice
    })
...
async function generate_donut_chart(div) {
  ...
  if (Object.keys(response).length != 0) {
    ...
    g.append("path")
      ...
      .attr("data-bs-html", "true")
      .attr("title", function (d) {
        let val = String(response[d.data]);
        if (
...
async function generate_line_chart(div) {
  ...
  const diff_days_reference = new Date(
    getDiffDays(div) * 1000 * 60 * 60 * 24,
  ).getDate();
  // Convert the recieved response to a javascript array usable by D3
  var data = [];
  for (k in response) {
    ...
    for (k2 in response_compare) {
      const diff_days = new Date(parseDate(k) - parseDate(k2)).getDate();
      if (diff_days == diff_days_reference) {
  ...
  if (div.dataset.days) {
    begin_date.setDate(new Date().getDate() - div.dataset.days);
  } else {
    const fromDate = document.getElementById("from-date");
    const toDate = document.getElementById("to-date");
  ...
  if (getDiffDays(div) >= 30) {
    ...
    end_date.setDate(1); // +1 to display the last tick
  }
...
function getDiffDays(div) {
  ...
  if (
    ...
    const begin_date = new Date(Date.parse(fromDate.value));
    const end_date = new Date(Date.parse(toDate.value));
...
function getDateFilterArg(div, compare_with_previous = false) {
  ...
  if (isNaN(parseInt(div.dataset.days))) {
    ...
    // data-days property onboarded in the HTML code
    begin_date.setDate(new Date().getDate() - parseInt(div.dataset.days));
  }

  if (compare_with_previous) {
    const diff_days = getDiffDays(div);
    begin_date.setDate(begin_date.getDate() - diff_days);
    end_date.setDate(end_date.getDate() - diff_days);
  }

  begin_date = begin_date.toISOString(true).slice(0, -5);
  end_date = end_date.toISOString(true).slice(0, -5);
...
function getOtherFiltersArg(with_attribute_only = false) {
  let filters = "";
