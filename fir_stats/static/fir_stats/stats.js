...
async function setIncidentCount(
  div,
  force_unit = false,
  with_attribute_only = false,
) {
  const date_filter = getDateFilterArg(div);
  if (!date_filter) {
    return false;
  }
  const other_filters = getOtherFiltersArg(force_unit, with_attribute_only);
...
// Enable/disable Selected attribute count when
